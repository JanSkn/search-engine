...
from cpp_utils import DocInfo, normalize_search_query
...
def test_index_metadata():
    ...
    # counting title + body, excluding stop words
...
class TestDocStore:
    def test_docstore_metadata(self):
        inverted_index = get_index()
        ...
        expected_docs = {
            ...
            10: DocInfo("http://example.com/10", "Title Eleven", snippet=""),
        }

        doc_store.query_terms = ["-1"]  # not needed here, but must be set
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "hello",
                "<b>Hello</b> - what is up? These are the first 15 words of the body. Now, we have ...",
            ),
            (
                "snippet",
                "... very long to see the effect of 2 potential different windows. Here begins the second <b>snippet</b>",
            ),
            (
                "hello body",
                "<b>Hello</b> - what is up? These are the first 15 words of the <b>body</b>. Now, we have a big gap until the next ...",
            ),
            (
                "hello snippet",
                "<b>Hello</b> - what is up? These are the first 15 ... potential different windows. Here begins the second <b>snippet</b>",
            ),
        ],
    )
    def test_docstore_snippets(self, query: str, expected_snippet: str):
        inverted_index = get_index()
        doc_store = inverted_index.doc_store

        DOC_ID = 10
        doc_store.query_terms = normalize_search_query(query)
        snippet = doc_store.get(DOC_ID).snippet
