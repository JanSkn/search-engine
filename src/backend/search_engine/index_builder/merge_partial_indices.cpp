...
std::vector<DictEntry> readDictionary(const std::string& dictFile,
...
std::vector<PostingEntry> readAndParsePosting(const std::string& postingsFile, uint64_t offset,
                                              uint64_t size) {
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        std::string dictFile =
            partialIndexDictDir + "/dictionary_" + std::to_string(partialIndexCount) + ".bin";
    ...
    for (size_t i = 0; i < partialIndexCount - 1; i++) {
        ...
        std::string postingsFile =
            partialIndexPostingsDir + "/postings_" + std::to_string(i) + ".bin";
        allDicts[i] = readDictionary(dictFile, postingsFile);
    }
    allDicts[partialIndexCount - 1] =
        readDictionary(partialIndexDictDir + "/dictionary_final.bin",
    ...
    struct HeapEntry {
        ...
        size_t dictIndex;
        size_t entryIndex;
        bool operator>(const HeapEntry& other) const { return term > other.term; }
    };
    ...
    while (!minHeap.empty()) {
        ...
        // add the current entry's posting
        {
            ...
            std::string postingsFile =
                (dictIndex == partialIndexCount - 1)
                    ? partialIndexPostingsDir + "/postings_final.bin"
                    : partialIndexPostingsDir + "/postings_" + std::to_string(dictIndex) + ".bin";

            postingsToMerge.push_back(
                readAndParsePosting(postingsFile, entry.offset, entry.postingSize));
        ...
        for (const auto& entry : mergedPostings) {
            postingSize +=
                sizeof(uint32_t) + sizeof(uint32_t) + entry.positions.size() * sizeof(uint32_t);
        }
        ...
        if (termsProcessed % 100000 == 0) {
            ...
            std::cout << "Processed " << termsProcessed << " terms, elapsed time: " << elapsed
                      << "s" << std::endl;
        }
    }
