...
class JsonlLTRDataset(Dataset):
    ...
    def __init__(
        self, path: Path, feature_order: List[str], max_docs: int | None = None
    ):
        self.path = Path(path)
...
@torch.no_grad()
def mrr_at_k(
    scores: torch.Tensor, labels: torch.Tensor, mask: torch.Tensor, k: int = 10
) -> float:
...
@torch.no_grad()
def evaluate(
    model: TinyLTRModel, loader: DataLoader, device: torch.device
) -> Dict[str, float]:
    model.eval()
    mrrs, ndcgs, losses = [], [], []
    for batch in loader:
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=20,
        help="truncate per query (see slides about truncation)",
    )
    ...
    ap.add_argument(
        "--out", type=str, default=str(_script_dir / "output" / "ltr_model.pt")
    )
    ...
    model = TinyLTRModel(
        num_features=num_features, hidden=args.hidden, dropout=args.dropout
    ).to(device)
