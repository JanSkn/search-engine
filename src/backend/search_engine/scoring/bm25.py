...
def _field_norm_tf(
    tf: int, *, field_len: int, avg_field_len: float, b_f: float
) -> float:
    """
    Field-normalized TF:
    ...
    *,
    ...
    for t in query_terms:
        ...
        term_idf_all.append(
            (
                t,
                float(
                    bm25_idf(num_docs, int(df), clamp_negative=cfg.clamp_negative_idf)
                ),
            )
        )
    ...
    for t, idf in term_idf.items():
        ...
        for d in cand_list:
            ...
            tf_norm = cfg.boost_title * _field_norm_tf(
                tf_title,
                field_len=_tlen(d),
                avg_field_len=avg_title_len,
                b_f=cfg.b_title,
            ) + cfg.boost_body * _field_norm_tf(
                tf_body, field_len=_blen(d), avg_field_len=avg_body_len, b_f=cfg.b_body
            )
            if tf_norm <= 0.0:
                continue
